...
/* Staff Terminal Styles */
.tnav { display: flex; justify-content: space-between; align-items: center; padding: 20px; border-bottom: 1px solid ${C.border}; }
.row { display: flex; align-items: center; }
.g10 { gap: 10px; }
.w100 { width: 100%; }
.col { display: flex; flex-direction: column; }
.wm { font-size: 16px; font-weight: 700; }
.btn-ic { width: 44px; height: 44px; border-radius: 12px; border: none; background: rgba(255,255,255,0.08); cursor: pointer; color: ${C.cream}; }
.btn-sm { padding: 8px 14px; font-size: 12px; border-radius: 8px; }
.btn-full { flex: 1; padding: 12px 16px; border-radius: 12px; border: none; cursor: pointer; font-weight: 600; }
.btn-bl { background: linear-gradient(135deg, ${C.blue}, ${C.navy}); color: ${C.cream}; }
.btn-or { background: linear-gradient(135deg, ${C.orange}, ${C.gold}); color: #000; }
.btn-gd { background: linear-gradient(135deg, ${C.orange}, ${C.gold}); color: #000; }
.lbl { font-size: 12px; font-weight: 600; color: rgba(240,237,230,0.6); letter-spacing: 0.5px; text-transform: uppercase; }
.inp { width: 100%; padding: 12px 14px; background: ${C.raised}; border: 1px solid ${C.border}; border-radius: 10px; color: ${C.cream}; font-size: 14px; font-family: 'Inter Tight', sans-serif; }
.inp:focus { outline: none; border-color: ${C.blue}; box-shadow: 0 0 20px rgba(0,122,193,0.3); }
...
function StaffTerminal(){
  ...
  const stamp = () => {
    ...
    try{
      ...
      if(!avail.length){dispatch({t:"TOAST",v:{msg:"All stamps collected! 🏆",icon:"🏆",type:"default"}});return;}
      const sid = avail[0];
      ...
      dispatch({t:"TOAST",v:{msg:`⚡ ${STATIONS[sid].full} — Stamped!`,icon:"✓",type:"default"}});
      Store.sync({event:"stamp_awarded",pid:info.id,sid,stationName:STATIONS[sid].full});
    }catch(e){
      dispatch({t:"TOAST",v:{msg:"Stamp error",icon:"⚠️",type:"default"}});
    }
  };
  ...
  const redeem = () => {
    ...
    try{
      ...
      dispatch({t:"TOAST",v:{msg:"Redeem error",icon:"⚠️",type:"default"}});
    }
  };
    ...
    if(scanning){
      const start = async () => {
        try{
          ...
          dispatch({t:"TOAST",v:{msg:"Camera access denied",icon:"📹",type:"default"}});
          setScanning(false);
        }      
      };
      start();
  ...
  const scanNFC = async () => {
    if(!("NDEFReader" in window)){
      dispatch({t:"TOAST",v:{msg:"NFC not supported on this device",icon:"📡",type:"default"}});
      return;
    }
    try{
      ...
      ndef.onreading = evt=>{
        try{
          ...
          setToken(text);
          lookup();
          dispatch({t:"TOAST",v:{msg:"✓ NFC read successful",icon:"✓",type:"default"}});
        }catch(e){
          dispatch({t:"TOAST",v:{msg:"NFC read error",icon:"⚠️",type:"default"}});
        }        
      };
    }catch(err){
      dispatch({t:"TOAST",v:{msg:"NFC permission denied",icon:"📡",type:"default"}});
    }
  };
  ...
  return(
    <div className="page" style={{paddingBottom:120,background:"transparent"}}>
      <div className="court-bg"/>
      <div style={{position:"relative",zIndex:2}}>
        <div className="tnav" style={{borderBottom:"1px solid "+C.border}}>
          <div className="row g10">
            <button className="btn-ic" onClick={back} style={{opacity:0.6}}>←</button>
            <div>
              <div className="wm">Staff Terminal</div>
              <div style={{fontSize:9,color:"rgba(240,237,230,0.38)",fontFamily:"'Inter Tight',sans-serif",letterSpacing:1.5,textTransform:"uppercase"}}>Fan Scanner</div>
            </div>
          </div>
          <button className="btn-sm" style={{background:redeemMode?"rgba(239,59,35,0.3)":"transparent",color:redeemMode?C.orange:"rgba(240,237,230,0.5)",border:"1px solid "+(redeemMode?"rgba(239,59,35,0.6)":"rgba(255,255,255,0.1)")}} onClick={()=>setRedeemMode(!redeemMode)}>
            {redeemMode?"🏆 Redeem":"Redeem Mode"}
          </button>
        </div>

        <div className="col g10 w100" style={{marginTop:24,borderRadius:20,overflow:"hidden",background:"rgba(0,29,58,0.8)",border:"1px solid rgba(0,122,193,0.3)",padding:16,margin:"24px 16px 0"}}>
          ...
          <div className="row g8">
            ...
            <button className="btn btn-full" style={{background:"rgba(0,122,193,0.15)",border:"1.5px solid rgba(0,122,193,0.4)",color:C.blue,fontWeight:700,fontSize:14}} onClick={scanNFC}>
              <span style={{fontSize:18}}>📡</span>Tap NFC
            </button>
          </div>
        </div>

        <div className="col g10 w100" style={{marginTop:16,padding:"0 16px"}}>
          ...
          <div className="row g8">
            ...
            <button className="btn btn-full btn-bl" onClick={lookup}>Go</button>
          </div>
        </div>

        {info && !info.error && (
          <div className="col g12 w100" style={{marginTop:20,borderRadius:18,background:"linear-gradient(135deg,rgba(0,122,193,0.15),rgba(253,185,39,0.08))",border:"1.5px solid rgba(253,185,39,0.35)",padding:16,margin:"0 16px",animation:`slideUp 0.4s ${spring.smooth}`}}>
            ...
            <div style={{width:"100%",height:4,background:"rgba(255,255,255,0.1)",borderRadius:20,overflow:"hidden"}}>
              <div style={{height:"100%",background:"linear-gradient(90deg,"+C.blue+","+C.gold+")",width:(stampCount/TOTAL)*100+"%",transition:"width 0.6s "+spring.smooth}} />
            </div>
            ...
            <div className="row g10" style={{marginTop:6}}>
              {!redeemMode ? (
                <>
                  <button className="btn btn-full btn-gd" onClick={stamp} disabled={stampCount>=TOTAL} style={{fontWeight:700}}>
                    <span style={{fontSize:16}}>⚡</span>Add Stamp
                  </button>
                  <button className="btn btn-full" style={{background:"rgba(34,212,106,0.2)",border:"1.5px solid rgba(34,212,106,0.4)",color:C.success,fontWeight:700}} onClick={()=>setRedeemMode(true)} disabled={info.redeemed}>
                    <span style={{fontSize:16}}>🏆</span>
                  </button>
                </>
              ) : (
                <>
                  <button className="btn btn-full btn-or" onClick={redeem} disabled={info.redeemed} style={{fontWeight:700,fontSize:15}}>
                    🏆 REDEEM PRIZE
                  </button>
        ...
        {info?.error && (
          <div className="col g10 w100" style={{marginTop:20,borderRadius:16,background:"rgba(229,62,62,0.12)",border:"1.5px solid rgba(229,62,62,0.4)",padding:14,margin:"0 16px",animation:`slideUp 0.3s ${spring.smooth}`}}>
            <div className="row g8">
        ...
        {!info && (
          <div className="col g16 w100" style={{marginTop:30,padding:"0 16px",marginBottom:20}}>
            <div className="row g10">
              ...
              <div style={{flex:1,borderRadius:14,background:"rgba(34,212,106,0.1)",border:"1px solid rgba(34,212,106,0.3)",padding:12,textAlign:"center"}}>
                ...
                <div style={{fontSize:22,fontWeight:900,color:C.success,marginTop:4}}>{state.db.metrics.redeems||0}</div>
              </div>
            </div>
          </div>
        )}
